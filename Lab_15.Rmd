...
TPS <- Exam1 %>% filter(math.score >= 70, reading.score >= 70, writing.score >= 70) %>% group_by(race.ethnicity) %>% summarize(count = n())
S <- sum(TPS$count)
TPS <- TPS %>% mutate(Prob = count/S) %>% select(race.ethnicity, Prob)
Exam1 <- Exam1 %>% 
  left_join(TPS, by = "race.ethnicity")
#graph the plot
ggplot(data = Exam1)+
 geom_col(aes(x = race.ethnicity ,y = Prob), position = "dodge")+
  facet_grid(rows = Exam1$gender)+
  ggtitle("Probability of Each Ethnicity Passing Each Exam Grouped by Gender")+
  xlab("Race/Ethnicity")+
  ylab("Probability")
```
###Findings:
