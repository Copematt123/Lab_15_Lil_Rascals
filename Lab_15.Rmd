...
This question is very important because not everyone has the equal access to such courses and tutors based on their social status. If this course has a legitimate effect on the test scores then it is an unfair advantage that should be offered in a way that gives everyone equal access. However if it does not truly affect the scores than it could be a waste of time.
```{r}
...
#### James' Reflection
  One my specific goals was to see the effects of tutoring on scores, which I believe I tested in this assignment above. Obviously this one data set can not be used as the model for all tutoring, however in this situation the prep-course(a form of tutoring) has a legitimate on the scores, that is not due to chance. A more general goal was to get a decent understanding of how to use R to analyze data. I believe I have certainly accomplished this goal. I now hove many ways do do so through graphs, some with linear models permutation tests. My other goals were more long term goals that I still have, but this class has brought me closer to being able to accomplish them.